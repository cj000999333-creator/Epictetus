// api/intake-chat.js
// Vercel serverless function for the Intake conversation.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (profile?.intake_completed) {
      return res.status(403).json({ error: "Intake already completed" });
    }

    const { userMessage, conversation } = req.body || {};

    if (!Array.isArray(conversation)) {
      return res.status(400).json({ error: "Missing conversation array" });
    }
...
// Retrieve relevant Stoic passages for the latest user message
    ...
    if (!epictetusRes.ok) {
      ...
      return res.status(epictetusRes.status).json({
        error: "Anthropic error (epictetus)",
        details: errText,
      });
    }
